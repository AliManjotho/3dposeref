...
def transform_world_to_camera(poses_set, cams, ncams=4 ):
    ...
    for t3dk in sorted( poses_set.keys() ):
      ...
      for c in range( ncams ):
        R, T, f, c, k, p, name, cf_length = cams[ (subj, c+1) ]
        ...
        cams[(subj, action, sname)] = cf_length
...
def unNormalizeData(normalized_data, data_mean, data_std, dimensions_to_ignore):
  ...
  """
  T = normalized_data.shape[0] # Batch size 64
  D = data_mean.shape[0] # Dimensionality 96

  orig_data = np.zeros((T, D), dtype=np.float32) # [64,96]
  dimensions_to_use = np.array([dim for dim in range(D)
                                if dim not in dimensions_to_ignore]) # 48
...
def project_to_cameras( poses_set, cams, ncams=4 ):
  ...
  for t3dk in sorted( poses_set.keys() ):
    ...
    for cam in range( ncams ):
      R, T, f, c, k, p, name, _ = cams[ (subj, cam+1) ]
